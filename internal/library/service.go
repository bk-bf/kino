...
func (s *Service) FetchShowEpisodeCount(ctx context.Context, show *domain.Show) error {
	count, err := s.client.GetShowEpisodeCount(ctx, show.ID)
	if err != nil {
		s.logger.Error("failed to fetch episode count", "error", err, "showID", show.ID)
		return err
	}
