...
func (m *Model) pushAndLoadColumn(spec columnLoadSpec, cursor int) *drillResult {
	...
	if cached := spec.getCached(); cached != nil {
		col.SetItems(cached)
		fetchCmd := m.updateInspector()
		if m.navPlan != nil {
			return &drillResult{
				AwaitKind: spec.awaitKind,
				AwaitID:   spec.awaitID,
				Cmd:       tea.Batch(m.advanceNavPlanAfterLoad(spec.awaitKind, spec.awaitID), fetchCmd),
			}
		}
		return &drillResult{AwaitKind: AwaitNone, Cmd: fetchCmd}
	}
	...
	case domain.Library:
		...
		if v.ID == playlistsLibraryID {
			...
			if cached, ok := m.Store.GetPlaylists(); ok {
				col.SetItems(cached)
				return &drillResult{AwaitKind: AwaitNone, Cmd: m.updateInspector()}
			}
