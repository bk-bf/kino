...
func (m Model) handleToggleInspector() (tea.Model, tea.Cmd) {
	...
	m.updateLayout()
	if !m.ShowInspector && m.Preview != nil {
		// Clear the image when hiding the inspector
		m.Preview.SetItem("", "", 0, "", 0, "")
		return m, nil
	}
	// Re-fetch the image for the current selection when showing the inspector
	...
	if handled {
		if selection != nil {
			if top := m.ColumnStack.Top(); top != nil {
				...
				_ = m.updateInspector() // cmd dropped intentionally (sort doesn't change item)
			}
		}
		return true, m, nil
	}
...
func (m Model) handleFilterTypingInput(msg tea.KeyMsg) (bool, Model, tea.Cmd) {
	...
	m.ColumnStack.UpdateTop(newCol)
	if oldCursor != top.SelectedIndex() {
		_ = m.updateInspector() // cmd dropped — search filter doesn't change item selection
	}
	return true, m, nil
}
