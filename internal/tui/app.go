...
import (
	...
	"github.com/bk-bf/kino/internal/config"
...
type Model struct {
	...
	// Image preview pane (nil = disabled)
...
func NewModel(
	...
	return Model{
		...
		ShowInspector:   true, // Show inspector (with image preview) by default
		UIConfig:        uiConfig,
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case MoviesLoadedMsg:
		...
		cmds = append(cmds, m.updateInspector())
	...
	case PlaylistDeletedMsg:
		...
		return m, tea.Batch(cmds...)
	case preview.ImageFetchedMsg:
		if m.Preview != nil {
			newPrev, cmd := m.Preview.Update(msg)
			*m.Preview = newPrev
			if cmd != nil {
				cmds = append(cmds, cmd)
			}
	...
	case ShowEpisodeCountFetchedMsg:
		// EpisodeCount was updated in-place on the show pointer; re-render inspector.
		if top := m.ColumnStack.Top(); top != nil {
			if current, ok := top.SelectedItem().(*domain.Show); ok && current == msg.Show {
				m.Inspector.SetItem(msg.Show)
			}
		}
		return m, tea.Batch(cmds...)
	}
...
// and returns any commands generated (e.g., preview image fetch).
func (m *Model) updateInspector() tea.Cmd {
	if top := m.ColumnStack.Top(); top != nil {
		item := top.SelectedItem()
		m.Inspector.SetItem(item)
		var cmds []tea.Cmd
		if show, ok := item.(*domain.Show); ok && show.EpisodeCount == 0 {
			cmds = append(cmds, FetchShowEpisodeCountCmd(m.LibraryService, show))
		}
		if m.Preview != nil {
			cmds = append(cmds, m.setPreviewItem(item))
		}
		return tea.Batch(cmds...)
	} else {
		m.Inspector.SetItem(nil)
		if m.Preview != nil {
			m.Preview.SetItem("", "", 0, "", 0, "")
		}
	}
	return nil
}

// setPreviewItem extracts metadata from a domain item and calls Preview.SetItem.
func (m *Model) setPreviewItem(item interface{}) tea.Cmd {
	if item == nil {
		return m.Preview.SetItem("", "", 0, "", 0, "")
	}
	switch v := item.(type) {
	case *domain.MediaItem:
		return m.Preview.SetItem(v.ID, v.Title, int32(v.Year), v.FormattedDuration(), float32(v.Rating), v.Summary)
	case *domain.Show:
		return m.Preview.SetItem(v.ID, v.Title, int32(v.Year), "", float32(v.Rating), v.Summary)
	default:
		return m.Preview.SetItem("", "", 0, "", 0, "")
	}
}
