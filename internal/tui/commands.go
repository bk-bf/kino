...
// SyncPlaylistsCmd syncs playlists and their items
...
// LogoutCmd simply quits the application (no server config to clear)
...
func CreatePlaylistCmd(svc *playlist.Service, title string, itemIDs []string) tea.Cmd {
	return func() tea.Msg {
		...
		pl, err := svc.CreatePlaylist(ctx, title, itemIDs)
		if err != nil {
...
// FetchShowEpisodeCountCmd fetches the total episode count for a show lazily.
func FetchShowEpisodeCountCmd(svc *library.Service, show *domain.Show) tea.Cmd {
	return func() tea.Msg {
		ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
		defer cancel()
		if err := svc.FetchShowEpisodeCount(ctx, show); err != nil {
			return ErrMsg{Err: err, Context: "fetching episode count"}
		}
		return ShowEpisodeCountFetchedMsg{Show: show}
	}
}
