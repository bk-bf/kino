...
type Inspector struct {
	...
	imageRows     int // blank rows to reserve at the top for the image overlay
}
...
// SetImageRows sets how many blank rows to reserve at the top for the image overlay.
// Call with 0 when no image is displayed.
func (i *Inspector) SetImageRows(n int) {
	i.imageRows = n
}
...
func (i Inspector) View() string {
	...
	// Calculate available space for body (also subtract image rows)
	availableForBody := i.maxVisible - len(headerLines) - len(footerLines) - i.imageRows
	...
	// Assemble: image placeholder + title + header zone + scroll-up indicator + visible body + padding + scroll-down indicator + footer zone
	var parts []string

	// Reserve blank rows at the top for the image overlay
	if i.imageRows > 0 {
...
func (i Inspector) renderShowInspector(show domain.Show, width int) inspectorContent {
	...
	if show.EpisodeCount > 0 {
		...
		header.WriteString(styles.DimStyle.Render("Progress: —"))
	}
...
func (i Inspector) renderSeasonInspector(season domain.Season, width int) string {
	...
	if season.EpisodeCount > 0 {
		...
		b.WriteString(styles.DimStyle.Render("Progress: —"))
	}
