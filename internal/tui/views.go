...
func (m Model) View() string {
	...
	if stackLen == 0 {
		...
		if layout.inspectorWidth > 0 {
			...
			// Tell the inspector how many rows to reserve at the top for the image overlay
			if m.Preview != nil {
				m.Inspector.SetImageRows(m.Preview.ImageRows())
			} else {
				m.Inspector.SetImageRows(0)
			}
	...
	// Append image overlay — always called so pending clear sequences are emitted
	// even when the inspector is hidden (needsClear may be true after toggling).
	if m.Preview != nil {
		layout := m.calculateColumnLayout(m.Width)
		listW := m.Width - layout.inspectorWidth
		view += m.Preview.ImageOverlay(m.Height, listW)
	}
